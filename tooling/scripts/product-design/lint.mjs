...
const pointerCursorOwnerPath = "packages/app/src/index.css";
const pointerCursorVar = "--honk-cursor-interactive";
...
// The Pointer cursor setting resolves to one variable in one file. Any other stylesheet that
// hardcodes the pointer, or redefines that variable, forks the affordance back apart.
function checkPointerCursorOwnership() {
  const violations = targets.flatMap(collectTextFiles).flatMap((relativePath) => {
    if (!relativePath.endsWith(".css")) return [];
    return readFileSync(join(repoRoot, relativePath), "utf8")
      .split(/\r?\n/u)
      .flatMap((line, index) =>
        /\bcursor\s*:\s*pointer(?:\s*!important)?\s*;/iu.test(line) ||
        (relativePath !== pointerCursorOwnerPath && line.includes(`${pointerCursorVar}:`))
          ? [`${relativePath}:${String(index + 1)}`]
          : [],
      );
  });
  if (violations.length === 0) return;
  console.error(
    `Do not hardcode cursor: pointer in CSS. Only ${pointerCursorOwnerPath} may resolve ${pointerCursorVar}:\n${violations.join("\n")}`,
  );
  process.exit(1);
}
