...
// Resolved per file from its actual `@stylexjs/stylex` import specifiers, so both
// `import * as stylex` + `stylex.create(...)` and `import { create }` + `create(...)`
// (including aliased locals) are recognized. Matching every `X.create(...)` instead
// would false-positive on Object.create and store factories.
const stylexBindingsByProgram = new WeakMap();

function stylexBindings(program) {
  const cached = stylexBindingsByProgram.get(program);
  ...
  for (const statement of program.body ?? []) {
    ...
    for (const specifier of statement.specifiers ?? []) {
      ...
      if (specifier.type === "ImportSpecifier" && STYLEX_CALLS.has(specifier.imported.name)) {
        named.add(specifier.local.name);
      }
    }
  }
  const bindings = { namespaces, named };
  stylexBindingsByProgram.set(program, bindings);
  return bindings;
}
...
function isStylexCall(node) {
  ...
  if (
    ...
    return stylexBindings(programOf(node)).namespaces.has(node.callee.object.name);
  }
  if (node.callee.type === "Identifier") {
    return stylexBindings(programOf(node)).named.has(node.callee.name);
  }
  return false;
}
...
function namesCssProperties(node) {
  if (node === null || node === undefined) return false;
  if (node.type === "Identifier") return node.name === "CSSProperties";
  if (node.type === "TSQualifiedName") return namesCssProperties(node.right);
  if (node.type === "TSTypeReference") return namesCssProperties(node.typeName);
  if (node.type === "TSTypeAnnotation") return namesCssProperties(node.typeAnnotation);
  // `Record<string, React.CSSProperties>` and unions still describe style objects.
  if (Array.isArray(node.types)) return node.types.some(namesCssProperties);
  if (Array.isArray(node.typeArguments?.params)) {
    return node.typeArguments.params.some(namesCssProperties);
  }
  if (Array.isArray(node.params)) return node.params.some(namesCssProperties);
  return false;
}

// A style object escapes StyleX in two sanctioned places: a `React.CSSProperties` constant handed
// to a Base UI `style` hatch, and an inline `style={{...}}` attribute. Both paint real pixels, so
// the token rules apply there exactly as they do inside `stylex.create`.
function isInsideStyleObject(node) {
  for (let current = node.parent; current != null; current = current.parent) {
    if (current.type === "JSXAttribute") {
      return current.name?.type === "JSXIdentifier" && current.name.name === "style";
    }
    if (current.type === "TSAsExpression" || current.type === "TSSatisfiesExpression") {
      if (namesCssProperties(current.typeAnnotation)) return true;
    }
    if (current.type === "VariableDeclarator" && namesCssProperties(current.id?.typeAnnotation)) {
      return true;
    }
...
const TOKEN_OWNED_PROPERTIES = (() => {
  ...
  add(
    ...
    length(
      "a spacing token (spaceVars, or the owning surface group: controlVars/sidebarVars/conversationVars/toastVars/proseVars)",
    ),
  );
  add(
    [
  ...
  add(
    ["fontSize", "lineHeight"],
    length("fontVars sizes (or the owning surface group's size/leading pair)"),
  );
  // Sizing and offsets carry as much layout intent as padding does, and StyleX coerces a bare
  // number to px, so a unitless literal is a raw length too. Most of these resolve to a justified
  // fixed intrinsic rather than a token; the point is that the justification becomes explicit.
  add(
    [
      "width",
      "minWidth",
    ...
    {
      group: "a sizing token when one means the same thing, or a named module constant",
      // Padding and color have tokens, so hoisting them into a const dodges the design system.
      // A dialog width or an avatar box has no token and should not get one: a per-surface measure
      // as a token is single-use pollution. Its correct expression is a name, which the compiler
      // checks, rather than a suppression comment, which nothing checks.
      allowNamedConstant: true,
      test: (value) => {
        if (typeof value === "number") return value === 0 ? undefined : String(value);
        return typeof value === "string" ? value.match(RAW_LENGTH)?.[0] : undefined;
      },
    },
  );
  add(["fontWeight"], {
...
const noRawValues = {
  create(context) {
    ...
    return {
      ...
      Property(node) {
        if (!isInsideStylexCall(node) && !isInsideStyleObject(node)) return;
        ...
        for (const entry of values) {
          ...
          if (arm.allowNamedConstant === true && entry.constName !== undefined) continue;
...
const noHardcodedPointerCursor = {
  create(context) {
    if (!isUiPackageFile(context) && !isAppSourceFile(context)) return {};
    let constants = new Map();
    ...
    return {
      ...
      Property(node) {
        const propertyName =
          node.key.type === "Identifier" && !node.computed ? node.key.name : staticString(node.key);
        if (propertyName !== "cursor") return;
        const values = [];
        collectStaticValues(node.value, constants, values, undefined);
        if (!values.some((entry) => entry.value === "pointer")) return;
        context.report({
          node,
          message:
            "Do not hardcode cursor: pointer — use interactive semantics and let the app-level Pointer cursor setting own the affordance.",
        });
      },
      JSXAttribute(node) {
        if (
          node.name.type !== "JSXIdentifier" ||
          node.name.name !== "className" ||
          !staticString(node.value)?.split(/\s+/u).includes("cursor-pointer")
        ) {
          return;
        }
        context.report({
...
export default {
  ...
  rules: {
    ...
    "design-no-hardcoded-pointer-cursor": noHardcodedPointerCursor,
