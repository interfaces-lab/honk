...
result with source, bounded checks, and user-provided rendered evidence when the task needs it.
...
- **Inspect source before requesting rendered evidence.** Review the component structure, CSS or
  StyleX, styling tokens, shared component contracts, and state logic before deciding whether
  user-provided rendered evidence is needed.
- **Do not open or control a browser or local app for QA or runtime logging.** A browser renderer is
  not the Electron runtime and cannot supply its process logs. Use bounded tests, typechecks, builds,
  source inspection, and visual evidence already supplied by the user. If new live visual proof is
  indispensable, stop and ask the user to capture or exercise the exact state. Never claim visual
...
| Review    | “Audit,” “critique,” “what’s wrong?”, screenshot review, or code review    | Inspect source, styles, and component contracts first. Use user-provided rendered evidence when visual guidance is needed; otherwise name the evidence limitation. Do not edit unless asked. |
...
### 2. Inspect source and load product context

Read the applicable `AGENTS.md` chain, supplied briefs and designs, and the product logic that
determines mutations, permissions, validation, errors, and side effects. For an existing interface,
inspect its component tree, CSS or StyleX, styling tokens, shared component APIs, responsive rules,
and state handling before requesting additional rendered evidence.
...
3. Decide from the source review whether the user's existing screenshots or observations provide the
   visual guidance the task needs.
4. If new live visual proof is indispensable, stop and ask the user to capture the exact viewport or
   exercise the exact interaction. Do not open a browser, control the local app, or launch a stack.
5. Verify behavior that can be established deterministically with focused component tests, including
   applicable keyboard order, focus movement, loading behavior, and pointer or touch contracts.
6. When applicable, cover long content, large values, constrained width, reduced motion, and
   localization or RTL risk in bounded checks.
7. State exactly which visual evidence came from the user and what was code-verified; do not imply
   visual verification from source review.
