...
## rule/pointer-cursor-setting-owns-affordance

**Status:** accepted
**Scope:** web product controls in `packages/app` and `packages/ui`.
**Rule:** Do not hardcode `cursor: pointer` in a component or call site. Use the correct interactive
element or ARIA role and let the app-level Pointer cursor preference apply the pointer affordance.
**Rationale:** A component-owned pointer cursor overrides the user's explicit interaction preference
and makes the Settings control dishonest.
**Evidence:** `packages/app/src/settings-general.tsx`, `packages/app/src/app-settings-store.ts`,
`packages/app/src/index.css`; deterministic rule `honk/design-no-hardcoded-pointer-cursor`.
**Accepted decision source:** explicit user direction, 2026-07-26.
**Exceptions:** Renderer-native cursor behavior and task-specific cursors such as text, resize, grab,
and drag remain owned by their interaction. Native links may keep browser-default link behavior.
Composite click surfaces that cannot honestly claim a button role may use the settings-owned
`data-cursor-pointer-control` hook.

Bad: set `cursor: "pointer"` in `Button`, `IconButton`, or a clickable row.
Good: render a semantic button or role and let the settings-owned root selector opt into the pointer.
