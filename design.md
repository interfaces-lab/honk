...
version: alpha
name: HonkKit
description: Honk's compact design system, Light theme (the Dark theme is documented at /design.dark.md).
sources:
...
HonkKit is Honk's design system for dense desktop product UI. The aesthetic is compact, neutral, and surface-led: restrained color, quiet strokes, and clear foreground hierarchy across app chrome, editor surfaces, chat, workbench, composer, menus, and terminals. Keep Honk's Pierre palette; make the token contract disciplined and Vercel-like.

This is the Light theme. The Dark theme uses the same token names with different computed values and lives at `/design.dark.md`.
...
`background-100` is the primary page and editor surface; `background-200` is the chrome/chat surface for subtle separation. The `neutral-*` staircase maps Honk's surface, stroke, and foreground roles into one readable ladder. Accent staircases are derived from existing Honk status tokens (`--primary`, `--success`, `--warning`, `--destructive`) and should not become new overlapping CSS variables unless product code repeatedly needs a specific step.

### Glass surfaces

Glass mode uses a fixed compact surface recipe. In light mode the base surfaces are sidebar `#F3F3F3`, chrome/chat `#F8F8F8`, editor/elevated `#FCFCFC`, foreground `#141414`, accent/focus `#3685BF`. When native vibrancy is available, the sidebar is the most translucent pane (`42%` base color over transparent), while chat and editor are denser panes (`84%` base color over transparent). Root glass remains `hsla(0, 0%, 100%, .16)`. Composer and chat bubbles stay near-opaque on `--honk-base-editor`; do not tint every nested card.
...
Hierarchy comes from tonal surfaces and borders first, so shadows stay subtle. Use the existing shadow ladder only: `soft`, `sm`, `base`, `lg`, `xl`, `card`, `popup`, `toolbar`, and `flat-ring`. Dark mode intentionally collapses most shadows; use light dividers and inset rings instead of inventing dark elevation.
