...
const WORKBENCH_START_TAB_ID = "start";
...
function openCodeWorkbenchStartHref(ref: OpenCodeSessionRef): string {
  return openCodeWorkbenchTabHref(ref, WORKBENCH_START_TAB_ID);
}
  ...
  try {
    ...
    const workbenchTabID =
      workbenchSegment === undefined
        ? undefined
        : requireIdentifier(decodeURIComponent(workbenchSegment), "workbench tab ID");
    const workbench: OpenCodeWorkbenchRouteTarget | undefined =
      workbenchTabID === WORKBENCH_START_TAB_ID
        ? Object.freeze({ type: "start" })
        : workbenchTabID !== undefined
          ? Object.freeze({ type: "tab", tabID: workbenchTabID })
...
function decodeBase64Url(value: string): string {
  if (!/^[A-Za-z0-9_-]+$/.test(value)) {
    throw new Error("Invalid base64url route segment.");
  }
