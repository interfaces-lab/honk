...
import { encodeBase64Url } from "./tab-route";
...
// local tabs can reference an origin that no longer exists. Rekeying moves the
// tabs, selection, remembered info, and closed stack from the stale server onto
// the live one instead of leaving them permanently unreachable.
function rekeyOpenCodeServerTabs(
  state: OpenCodeTabState,
  from: OpenCodeServerKey,
  to: OpenCodeServerKey,
): OpenCodeTabState {
  if (from === to) return state;

  const movedKeys = new Map<OpenCodeTabKey, OpenCodeTabKey>();
  const seen = new Set<OpenCodeTabKey>();
  const tabs: OpenCodeTab[] = [];
  for (const tab of state.tabs) {
    const next = tab.server === from ? withTabServer(tab, to) : tab;
    const key = openCodeTabKey(next);
    if (next !== tab) movedKeys.set(openCodeTabKey(tab), key);
    if (seen.has(key)) continue;
    seen.add(key);
    tabs.push(next);
  }
  if (movedKeys.size === 0 && !state.closed.some((entry) => entry.tab.server === from)) {
    return state;
  }

  const info: Record<string, OpenCodeTabInfo> = {};
  for (const [key, value] of Object.entries(state.info)) {
    const nextKey: string = movedKeys.get(key as OpenCodeTabKey) ?? key;
    if (info[nextKey] !== undefined) continue;
    info[nextKey] = nextKey === key ? value : rekeyedInfo(value, from, to);
  }

  const closed = state.closed.map((entry) =>
    entry.tab.server === from
      ? Object.freeze({
          tab: openCodeSessionTab(openCodeSessionRef(to, entry.tab.sessionID)),
          index: entry.index,
        })
      : entry,
  );
  ...
  return freezeState({
    ...
    tabs,
    activeKey:
      state.activeKey === null ? null : (movedKeys.get(state.activeKey) ?? state.activeKey),
    recentKey:
      state.recentKey === null ? null : (movedKeys.get(state.recentKey) ?? state.recentKey),
    info,
    closed,
  });
}

function withTabServer(tab: OpenCodeTab, server: OpenCodeServerKey): OpenCodeTab {
  if (tab.type === "session") {
    return openCodeSessionTab(openCodeSessionRef(server, tab.sessionID));
  }
  return Object.freeze({ ...tab, server });
}

// Remembered routes embed the origin in their `/server/<segment>/` prefix.
function rekeyedInfo(
  info: OpenCodeTabInfo,
  from: OpenCodeServerKey,
  to: OpenCodeServerKey,
): OpenCodeTabInfo {
  if (info.route === undefined) return info;
  return Object.freeze({
    ...info,
    route: info.route.replace(
      `/server/${encodeBase64Url(from)}/`,
      `/server/${encodeBase64Url(to)}/`,
    ),
  });
}
