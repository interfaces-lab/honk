...
describe("task child-session projection", () => {
  ...
  // The thread's first sidekick call carries no session id while it runs, so the agent name is
  // all it has. An abandoned sidekick session shares that name, and only the id another call
  // already names tells the two apart.
  it("links the first in-flight call to the child no other call claims", () => {
    const links = projectTaskChildLinks({
      parts: [
        task({ id: "part_abandoned", metadataID: "ses_abandoned" }),
        task({ id: "part_first", agent: "honk-sidekick-medium", status: "running" }),
      ],
      children: [child({ id: "ses_abandoned" }), child({ id: "ses_fresh", status: "running" })],
      parentSessionID: "ses_parent",
      server: local.key,
    });

    expect(links).toMatchObject([
      { partID: "part_abandoned", ownsLiveState: true, state: "done" },
      { partID: "part_first", ownsLiveState: true, state: "running" },
    ]);
    expect(links[1]?.child.id).toBe("ses_fresh");
  });

  it("still refuses to guess when every matching child is unclaimed", () => {
    expect(
      projectTaskChildLinks({
        parts: [task({ id: "part_first", agent: "honk-sidekick-medium", status: "running" })],
        children: [child({ id: "ses_one" }), child({ id: "ses_two" })],
        parentSessionID: "ses_parent",
        server: local.key,
      }),
    ).toEqual([]);
