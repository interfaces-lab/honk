...
const ITEM_KEYS = ["block:0", "block:1", VIRTUAL_CONVERSATION_SPACER_KEY] as const;
...
function createVirtualizer(input?: {
  ...
  virtualizer.shouldAdjustScrollPositionOnItemSizeChange = (item, delta, instance) =>
    shouldAdjustConversationScrollPosition(item, delta, instance, input?.stickyIndexes ?? []);
  return { scrollToFn, virtualizer };
}

describe("virtual conversation resize policy", () => {
  test("compensates first measurements even while scrolling up", () => {
    const subject = createVirtualizer({ direction: "backward" });

    // block:0 is above the viewport (start 0 < offset 100) and unmeasured, so
  ...
  test("skips re-measurements while scrolling up", () => {
    const subject = createVirtualizer({ direction: "backward" });
    subject.virtualizer.itemSizeCache.set("block:0", 120);
