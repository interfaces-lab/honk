...
export function resolveTaskChildSession(input: {
  ...
  // Child ids some other task call names outright. A thread's first sidekick call names none:
  // the host plugin drops `task_id` when there is no session to resume, and the task tool only
  // publishes `metadata.sessionId` once the call settles, so until then the agent name is the
  // only evidence it has. Skipping spoken-for ids keeps that fallback unambiguous when an
  // abandoned sidekick session still shares the agent.
  ...
  // An explicit id is authoritative. One that resolves to nothing means the child is gone or not
  // in the inventory yet, and inferring a substitute would point the row at another subagent.
  ...
  const candidates = children.filter(
    (child) => child.agent === agent && input.claimedSessionIDs?.has(child.id) !== true,
  );
