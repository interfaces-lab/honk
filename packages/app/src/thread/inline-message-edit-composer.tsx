...
// The edit composer keeps a fixed single-line height before it grows.
const EDITOR_MIN_HEIGHT = "36px";
const EDITOR_MAX_HEIGHT = "72px";
...
const styles = stylex.create({
  ...
  // Cursor aliases the human-message content inset into edit mode, so switching from the
  // read surface to the editor preserves the text and attachment origin.
  editor: {
...
export function InlineMessageEditComposer({
  ...
  const threadMode = useThreadMode(runtime.tabKey, coreMode);
  ...
  const isSessionRunning = useSessionWatchSelector(
    runtime.ref,
    (snapshot) => (snapshot.state?.activity ?? "idle") !== "idle",
  );
    ...
    <>
      ...
      >
        <PromptEditor
          ...
          attachmentsStyle={styles.attachments}
          handleRef={editorRef}
        />
