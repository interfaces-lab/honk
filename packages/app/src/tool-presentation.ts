...
function toolVerb(part: ToolPart, state: ToolCallState = toolLineState(part)): string {
  ...
  switch (part.tool) {
    ...
    case "plan_submit":
    case "honk_plan_submit":
      return isRunning ? "Recording plan" : "Plan recorded";
    case "skill":
      return isRunning ? "Loading" : "Loaded";
    default: {
      // The MCP generation of Honk's own tools is the same call under a
      // prefixed name; label it exactly as its plugin predecessor.
      const name = humanizeToolName(part.tool.replace(/^honk_/, ""));
