...
describe("session prompt", () => {
  ...
  // A client-minted ID can sort above the sidecar's ascending assistant IDs, which
  // pins the session's latest user message and traps the runner in its turn loop.
  it("leaves the message ID to the sidecar when the caller has none", async () => {
    const value = client(() => Promise.resolve());

    await sendSessionPrompt(value, "session-1", { text: "Audit the chat view" });

    expect(value.sessions.prompt).toHaveBeenCalledWith(
      openCodeSessionRef(server.key, "session-1"),
      {
        prompt: { text: "Audit the chat view" },
      },
    );
  });
