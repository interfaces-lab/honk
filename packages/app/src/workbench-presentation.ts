...
import { IconFileBend, IconWindowSquare } from "@honk/ui/icons";
...
function workbenchPresentation(input: {
  ...
  const toolTabs = workbenchToolTabs(input.planPresent, input.tasks);
  ...
  const tabLabels = new Map(
    input.managedTabs.map(
      (tab) =>
        [tab.id, tabLabel(tab, input.managedTabs, toolTabs, sideChatLabels.get(tab.id))] as const,
    ),
  );
