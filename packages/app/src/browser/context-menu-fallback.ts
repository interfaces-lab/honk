...
export function showContextMenuFallback<T extends string>(
  ...
  return new Promise<T | null>((resolve) => {
    ...
    menu.className =
      "fixed z-[10000] min-w-36 rounded-md border border-border bg-popover py-1 shadow-xl animate-in fade-in zoom-in-95";
    ...
    for (const item of items) {
      ...
      btn.className = isDisabled
        ? "flex w-full items-center gap-2 px-3 py-1.5 text-left text-detail text-muted-foreground/60 cursor-not-allowed"
        : isDestructiveAction
          ? "flex w-full items-center gap-2 px-3 py-1.5 text-left text-detail text-destructive hover:bg-accent cursor-default"
          : "flex w-full items-center gap-2 px-3 py-1.5 text-left text-detail text-popover-foreground hover:bg-accent cursor-default";
      if (!isDisabled) {
