...
import { readRemoteServerRegistry } from "./server-registry";
...
import { terminalTabID } from "./workbench-tab-store";
...
let terminalOpenInstalled = false;
...
// The desktop sidecar binds a fresh ephemeral port each launch, so restored
// tabs can reference a local origin no server will ever register again. Adopt
// them onto the bound local server as soon as it connects.
subscribeOpenCodeServers(adoptStaleLocalServerTabs);
...
// Home directory for a server, used to abbreviate previewed paths with "~".
// Null until resolvePath resolves; callers fall back to the unabbreviated path.
export function useServerHome(server: OpenCodeServerKey): string | null {
  return useSyncExternalStore(
    subscribe,
    () => serverHomes[server] ?? null,
    () => null,
  );
}
...
export function installTerminalOpenBridge(): void {
  if (terminalOpenInstalled || typeof window === "undefined") return;
  terminalOpenInstalled = true;
  subscribeDesktopTerminalOpen((request) => {
    const ref = sessionRefForInput(request.threadId);
    if (ref === null) return;
    // Target this exact terminal: the tab and the agent's background job share the id.
    openCanonicalWorkbenchTool(ref, terminalTabID(request.terminalId));
  });
}
...
export const actions = {
  ...
  openDraft(input?: { readonly server?: OpenCodeServerKey; readonly directory?: string }): void {
    ...
    openNewDraft(client, input?.directory);
  },

  openNew(input?: OpenNewThreadInput, options?: OpenNewThreadOptions): void {
    if (input !== undefined) {
      void actions.submitNew(input, options);
      return;
    }
  ...
  submitNew(input: OpenNewThreadInput, options?: OpenNewThreadOptions): Promise<boolean> {
    const client =
      input.server === undefined ? getBoundOpenCodeClient() : getOpenCodeClient(input.server);
    if (client === null) {
      showConnectionError("The OpenCode connection is not ready yet.");
    ...
    rememberServer(client.server);
    return createAndOpenThread(client, input.prompt ?? "", input, options);
  },
...
function adoptStaleLocalServerTabs(): void {
  const bound = getBoundOpenCodeClient();
  if (bound === null || bound.server.kind !== "local") return;
  const state = windowTabStore.getSnapshot();
  const staleServers = new Set(
    [...state.tabs, ...state.closed.map((entry) => entry.tab)]
      .map((tab) => tab.server)
      .filter(
        (server) =>
          server !== bound.server.key &&
          getOpenCodeClient(server) === null &&
          createOpenCodeServer({ origin: server }).kind === "local",
      ),
  );
  if (staleServers.size === 0) return;
  // A user-added server can also live on a loopback origin; leave its tabs alone.
  const remoteServers = new Set(
    readRemoteServerRegistry().servers.map((record) => openCodeServerKey(record.origin)),
  );
  const previousActiveKey = state.activeKey;
  for (const server of staleServers) {
    if (remoteServers.has(server)) continue;
    windowTabStore.actions.rekeyServer(server, bound.server.key);
    if (servers.delete(server)) publishProjection();
  }

  const next = windowTabStore.getSnapshot();
  if (controller === null || next.activeKey === null || next.activeKey === previousActiveKey) {
    return;
  }
  const active = next.tabs.find((tab) => openCodeTabKey(tab) === next.activeKey);
  const ref = active === undefined ? null : openCodeTabSessionRef(active);
  if (active === undefined || ref === null) return;
  // The router already resolved the stale origin's href; swap in the adopted route.
  const remembered = next.info[next.activeKey]?.route;
  const parsed = remembered === undefined ? null : parseOpenCodeTabHref(remembered);
  const href =
    remembered !== undefined &&
    parsed?.type === "session" &&
    parsed.ref.server === ref.server &&
    parsed.ref.sessionID === ref.sessionID
      ? remembered
      : openCodeTabHref(active);
  controller.actions.openSessionRoute(ref, href, { replace: true });
}
...
// `tabID` is a workbench tab ID: a tool kind such as "browser", or a resource-scoped
// ID such as `terminal:<id>` when one specific resource must be targeted.
function openCanonicalWorkbenchTool(
  ref: OpenCodeSessionRef,
  tabID: string,
  options?: { readonly activate?: boolean },
): void {
  const href = openCodeWorkbenchTabHref(ref, tabID);
  if (controller === null) {
    windowTabStore.actions.openSession(ref, options);
    windowTabStore.actions.rememberSessionRoute(ref, href);
  } else {
...
function openNewDraft(client: OpenCodeClient, directory?: string): void {
  const state = windowTabStore.getSnapshot();
  const active = state.tabs.find((tab) => openCodeTabKey(tab) === state.activeKey);
  const activeDirectory =
    active?.server !== client.server.key
      ? undefined
      : active.type === "draft"
        ? active.location.directory
        : state.info[openCodeSessionTabKey(openCodeSessionRef(active.server, active.sessionID))]
            ?.directory;
  const configuredDirectory =
    directory ?? getAppSettings().defaultProjectDirectory ?? activeDirectory;
  if (configuredDirectory !== undefined) {
    openDraftAtLocation(client, openCodeLocationRef({ directory: configuredDirectory }));
    return;
  }
  ...
  void client
    .resolveLocation()
    .then((resolved) => {
      openDraftAtLocation(client, openCodeLocationRef({ directory: resolved.project.directory }));
    })
    .catch((error: unknown) => {
      const message = errorMessage(error);
      toastActions.add({
...
function openDraftAtLocation(client: OpenCodeClient, location: OpenCodeLocationRef): void {
  const input = {
