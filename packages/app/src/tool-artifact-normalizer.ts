...
type ToolDiffArtifactFile = ToolArtifactFile & {
  readonly patch: string;
};

type ToolDiffArtifact = {
  readonly kind: "diff";
  readonly files: readonly ToolDiffArtifactFile[];
};
...
function applyPatchArtifact(part: ToolPart): ToolDiffArtifact | undefined {
  ...
  if (patches !== undefined && patches.length > 0) {
    return {
      kind: "diff",
      files: patches.map((entry) => ({
        ...patchFile(entry.path, entry.patch, entry.file),
        patch: entry.patch,
      })),
    };
  }
...
function editDiffArtifact(part: ToolPart): ToolDiffArtifact | undefined {
  ...
  return {
    kind: "diff",
    files: [{ ...patchFile(path, normalized, fileDiff ?? metadata), patch: normalized }],
  };
}
