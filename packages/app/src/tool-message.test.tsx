...
describe("tool artifact normalization", () => {
  ...
  it("preserves every apply_patch file instead of flattening it to a file summary", () => {
    ...
    expect(artifact.files[0]?.patch).toContain("+++ b/src/a.ts");
    expect(artifact.files[1]?.patch).toContain("+++ b/src/b.ts");
  });
...
describe("tool artifact rendering", () => {
  ...
  it("renders every file in an OpenCode multi-file apply_patch after context trimming", () => {
    const part = completedTool("apply_patch", {}, "Done", {
      files: [
        {
          relativePath: "src/old.ts",
          patch: [
            "Index: /repo/src/old.ts",
            "===================================================================",
            "--- /repo/src/old.ts",
            "+++ /repo/src/old.ts",
            "@@ -1,3 +0,0 @@",
    ...
    const html = renderToStaticMarkup(<ToolMessage part={part} />);

    expect(renderable?.kind).toBe("files");
    if (renderable?.kind !== "files") return;
    expect(renderable.files.map((file) => file.name)).toEqual([
      "/repo/src/old.ts",
      "/repo/src/current.ts",
    ]);
    expect(html).toContain('data-pierre-tool-diff=""');
    expect(html).not.toContain("Showing raw patch");
  });
  ...
  it("keeps a task failure available when no child transcript exists", () => {
    ...
    expect(html).toContain('data-work-output-preview="false"');
