...
} from "./subagents/subagent-tray";

describe("shouldPresentSubagentTray", () => {
  it("allows a focused subagent tray whenever the caller marks the tray visible", () => {
    const activeThreadId = ThreadId.make("thread-1");

    expect(
      shouldPresentSubagentTray({
        activeThreadId,
        trayActiveThreadId: activeThreadId,
        hasFocus: true,
        visible: true,
      }),
    ).toBe(true);
  ...
  it("keeps inline edit and inactive-thread trays hidden", () => {
    expect(
      shouldPresentSubagentTray({
        activeThreadId: ThreadId.make("thread-1"),
        trayActiveThreadId: ThreadId.make("thread-1"),
        hasFocus: true,
        visible: false,
      }),
    ).toBe(false);
    ...
    expect(
      shouldPresentSubagentTray({
        activeThreadId: ThreadId.make("thread-1"),
        trayActiveThreadId: ThreadId.make("thread-2"),
        hasFocus: true,
        visible: true,
      }),
    ).toBe(false);
...
describe("hasRenderableSubagentSnapshotItem", () => {
  ...
  it("uses raw snapshot message text instead of truncated detail text", () => {
    const fullText =
