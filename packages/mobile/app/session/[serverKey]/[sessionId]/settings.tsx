import { openCodeServerKey, openCodeSessionRef, type OpenCodeSessionInfo } from "@honk/opencode";
...
import { ConnectionUnavailableState } from "../../../../src/connection-unavailable-state";
...
export default function SessionSettingsRoute(): React.ReactElement {
  ...
  const server = remote.servers.find((candidate) => candidate.descriptor.key === serverKey) ?? null;
  if (server === null) return <Redirect href="/(tabs)/home" />;
  if (client === null) {
    return (
      <Page>
        <ConnectionUnavailableState
          fallbackDetail={`Honk will reopen these settings when ${server.descriptor.label} is reachable.`}
          server={server}
        />
      </Page>
    );
        ...
        {info === null ? null : (
          <>
            <View style={{ gap: theme.metrics.space.compactGap }}>
              ...
              <DetailText>
                Details only. Honk cannot rename, archive, or delete a session.
              </DetailText>
            </View>
