...
export function ThreadRow({
  ...
  const emphasized = !session.cached && (session.needsAttention || session.status === "failed");
  const active = !session.cached && (emphasized || session.status === "running");
  ...
  return (
    ...
    >
      ...
      <ListRow.Content>
        ...
        <ListRow.Description>
          {session.cached || active ? `${statusLabel(session)} · ` : ""}
