...
import {
  ...
  toLocations,
  toToolKind,
} from "../../../src/provider/acp/AcpTool.ts";

// Ported from anomalyco/opencode packages/opencode/test/acp/tool.test.ts.
describe("AcpTool", () => {
  it("maps OpenCode tool ids to ACP tool kinds", () => {
  ...
  it("extracts file locations from tool input", () => {
  ...
  it("builds completed content with text, edit diffs, and image attachments", () => {
  ...
  it("omits edit diffs until old and new text fields exist", () => {
    expect(
  ...
  it("builds completed raw output with optional metadata and attachments", () => {
    const attachments = [
      {
        type: "file",
        mime: "image/jpeg",
  ...
  it("extracts image attachments only from data URLs", () => {
    const attachments = [
      {
        mime: "image/webp",
  ...
  it("reads shell output snapshot from string metadata output", () => {
