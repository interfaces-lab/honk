...
type OpenCodeSessionTranscriptSources = {
  ...
  /** Set when the plane fetch failed and the transcript rendered without it. */
  readonly persistedError?: string;
  readonly projectedError?: string;
};
...
function projectOpenCodeTranscript(
  ...
  failures?: { readonly persisted?: string; readonly projected?: string },
): OpenCodeSessionTranscript {
  const visiblePersisted = persisted.filter(hasPersistedTranscriptContent);
  const projectedTranscript = projectMessages(
    info,
    projected,
    findLastPersistedUserMessageID(visiblePersisted),
  );
  const messages = new Map<string, Message>();
  const parts = new Map<string, Part>();

  for (const entry of visiblePersisted) {
  ...
  return Object.freeze({
    ...
    sources: Object.freeze({
      ...
      ...(failures?.persisted !== undefined ? { persistedError: failures.persisted } : {}),
      ...(failures?.projected !== undefined ? { projectedError: failures.projected } : {}),
    }),
  });
}

function hasPersistedTranscriptContent(entry: OpenCodePersistedMessage): boolean {
  // promptAsync may create the user message before the message endpoint returns its parts.
  return entry.info.role !== "user" || entry.parts.length > 0;
}
...
function projectMessages(
  ...
  for (const message of source) {
    // Every SessionMessage variant is handled below; agent-switched and
    // model-switched are session metadata and intentionally have no transcript
    // row, so rendered counts can be lower than the projected source count.
    if (message.type === "agent-switched" || message.type === "model-switched") {
      continue;
    }

    if (message.type === "synthetic" || message.type === "system") {
      messages.push(projectServiceMessage(info, message.id, message.time.created, parentID));
      parts.push({
        id: `${message.id}:text`,
        sessionID: info.id,
    ...
    if (message.type === "shell") {
      messages.push(
        projectServiceMessage(
          ...
          parentID,
          message.time.completed === undefined ? undefined : { completed: message.time.completed },
        ),
      );
      parts.push({
        id: `${message.id}:shell`,
...
/** Server-authored transcript rows (synthetic, system, shell) render as assistant turns. */
function projectServiceMessage(
  info: OpenCodeSessionInfo,
  id: string,
  created: number,
  parentID: string,
  ...
  return {
    ...
    time: {
      created,
      ...(time?.completed !== undefined ? { completed: time.completed } : {}),
    },
    parentID,
    modelID: model.id,
    providerID: model.providerID,
