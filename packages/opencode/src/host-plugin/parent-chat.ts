import type { PluginInput, PluginMessageGroup, ToolDefinition, ToolResult } from "./types";
...
export function createParentChatTool(plugin: PluginInput): ToolDefinition {
  return {
    description:
      "Reads the parent conversation for this side chat. Use it to pull parent messages you have not seen, or to search the current parent transcript. Pass an empty query for the latest transcript.",
    args: {
      query: {
        type: "string",
    ...
    async execute(args, context): Promise<ToolResult> {
      ...
      // Side chats are ordinary child sessions; the parent link is OpenCode's own parentID.
      const parentID = currentResult.data.parentID;
