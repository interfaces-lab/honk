  ...
  return (pi) => {
    ...
    pi.registerTool(
      defineTool({
        ...
        promptGuidelines: [
          "Before calling apply_patch, read every file you will update; do not guess context from search snippets.",
          "Use apply_patch for text-file edits, creates, deletes, and moves.",
          "Narrow apply_patch patches to the user's requested change. Do not reformat, refactor, or touch adjacent code unless it is required for that change.",
          "The apply_patch input must be a full Codex patch wrapped in *** Begin Patch and *** End Patch markers.",
          "Use *** Add File, *** Delete File, and *** Update File sections; Add File content lines must start with +.",
          "Group related edits into one patch when the changed files are part of the same fix.",
          "For Update File hunks, include enough unchanged context to uniquely identify the target: at least 3 lines above and below by default, and 5-10 lines for repeated or ambiguous blocks.",
          "Use @@ headers naming the class, function, or nearby anchor when context alone may match multiple locations; use multiple @@ anchors for deeply repeated code.",
          "For insert-only hunks, include a nearby unchanged context line; avoid unanchored insertions.",
          "If you only read part of a file, read more instead of guessing the surrounding patch context.",
          "Keep indentation and whitespace in context lines exactly as read from the file; do not reindent unrelated lines.",
          "When editing conflict markers, keep marker length consistent with the file's existing markers.",
