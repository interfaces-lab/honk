...
function InputGroup({
  ...
  return (
    <InputGroupContext.Provider value={{ disabled, size }}>
      <div
        className={cn(
          "relative inline-flex min-h-7 w-full min-w-0 items-center rounded-multi-control border border-multi-stroke-tertiary bg-multi-bg-quinary text-multi-fg-primary shadow-none transition-colors duration-150 ease-out has-focus-visible:border-multi-stroke-focused has-focus-visible:ring-1 has-focus-visible:ring-multi-stroke-focused/30 has-aria-invalid:border-multi-stroke-red-primary has-disabled:opacity-64 motion-reduce:transition-none",
          size === "lg" && "min-h-8",
          className,
        )}
...
function InputGroupAddon({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      className={cn(
        "inline-flex h-full shrink-0 items-center gap-1 px-2 text-body text-multi-fg-tertiary [&_svg]:size-4 [&_svg]:shrink-0",
        className,
      )}
