...
// Exception: rows that stand for a live session (subagents) opt into the shared status Matrix via
// workingGlyph — the same working/attention glyph as tabs and the sidebar, not a per-call icon.
...
interface ToolCallLineProps {
  ...
  statusGlyphVariant?: "working" | "attention" | undefined;
...
const styles = stylex.create({
  ...
  workingGlyph: {
    ...
    flexShrink: 0,
    height: fontVars["--honk-leading-heading"],
    color: "var(--_verb-color)",
...
function ToolCallLine({
  ...
  const shouldShimmer = isRunning && !(workingGlyph && statusGlyphVariant === "attention");
  ...
  const primaryContent = (
    <>
      <span
        {...stylex.props(
          styles.verb,
          shouldShimmer && supportingText === undefined && toolCallShimmer,
        )}
      >
        {verb}
      </span>
  ...
  const content = (
    <>
      {workingGlyph && isRunning && (
        <span
          data-tool-call-status-glyph={statusGlyphVariant}
          {...stylex.props(
            styles.workingGlyph,
            supportingText !== undefined && styles.stackedWorkingGlyph,
          )}
        >
          <Matrix grid={4} variant={statusGlyphVariant} isActive />
        </span>
      )}
      {supportingText === undefined ? (
        ...
        <span {...stylex.props(styles.textStack)}>
          ...
          <span {...stylex.props(styles.supportingText, shouldShimmer && toolCallShimmer)}>
            {supportingText}
          </span>
        </span>
      )}
