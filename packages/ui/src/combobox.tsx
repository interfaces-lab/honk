...
interface ComboboxProps {
  ...
  // Trigger content for the default field-style trigger; unused when `trigger` is set.
  ...
  // Observes the live search text so callers can fetch async results (e.g. server-side
  // fuzzy file search) and feed them back through a pinned group.
  readonly onQueryChange?: (query: string) => void;
  // Replaces the field-style trigger chrome entirely (e.g. an IconButton quick-open
  // affordance). The element receives the trigger wiring via Base.Trigger's render prop.
...
const COMBOBOX_FIELD_MIN_WIDTH = "120px";
...
// Shared edge gutter with the menu/picker popups so every dropdown insets its rounded
// row highlight the same distance from the popup wall.
const SECTION_EDGE_PAD = controlVars["--honk-control-menu-pad"];
...
// search magnifier up with the inset option leading icons (edge gutter + option pad).
const SECTION_INLINE_PAD = `calc(${SECTION_EDGE_PAD} + ${controlVars["--honk-control-pad-sm"]})`;
...
const sx = stylex.create({
  ...
  triggerNeutral: {
    minWidth: COMBOBOX_FIELD_MIN_WIDTH,
    boxShadow: {
  ...
  search: {
    ...
    flexShrink: 0,
    gap: controlVars["--honk-control-gap"],
    // Search-row inset so the input clears the popup's rounded top corner (compact-menu intrinsic).
  ...
  groupLabel: {
    paddingInline: controlVars["--honk-control-pad-sm"],
    // oxlint-disable-next-line honk/design-no-raw-values -- 3px is the compact picker group-label block inset; no spacing or control-padding token owns it
    paddingBlock: "3px",
    color: colorVars["--honk-color-text-muted"],
...
function Combobox({
  ...
  return (
    ...
    >
      {trigger !== undefined ? (
        <Base.Trigger render={trigger} aria-label={accessibilityLabel} title={title} />
      ) : (
        <Base.Trigger
