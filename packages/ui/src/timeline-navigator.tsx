...
// Timeline geometry stays fixed so user-message stops remain a quiet navigation aid. The rail
// pins to the thread panel's inline-start edge, so it stays narrow enough that its hover strip
// never blankets transcript content in squeezed layouts.
const TIMELINE_RAIL_WIDTH_PX = 24;
const MAX_PREVIEW_WIDTH_PX = 320;
const RAIL_WIDTH = `${String(TIMELINE_RAIL_WIDTH_PX)}px`;
...
const dynamic = stylex.create({
  ...
  stopHeight: (itemCount: number) => ({
    height: `min(${controlVars["--honk-control-h-md"]}, calc(100% / ${String(itemCount)}))`,
  }),
});
...
export { TIMELINE_RAIL_WIDTH_PX, TimelineNavigator };
