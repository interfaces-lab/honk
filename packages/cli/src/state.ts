...
const decodeHostState = (value: unknown): HostState | null => {
  ...
  // A host.json written before server identity existed must still restore its admin secret and
  // devices, so a missing id mints a new one instead of failing the whole decode.
  return {
    version: 1,
    pid,
    adminSecret,
    serverId: typeof serverId === "string" && serverId.length > 0 ? serverId : freshServerId(),
    origin,
    publicUrl,
    upstreamOrigin,
    cwd,
...
export const freshServerId = (): string => randomBytes(16).toString("base64url");
