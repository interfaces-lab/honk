...
// The opencode binary doubles as the runtime for Honk's stdio MCP server.
const writeHonkOpencodeConfig = async (
  statePath: string,
  ...
  const claudeFastWrapperPath = resolve(
    pluginDirectory,
  ...
  if (process.platform !== "win32") {
    await chmod(claudeFastWrapperPath, 0o755);
  }
  const config = buildHonkOpencodeConfig(pluginPath, {
    claudeCliPath: claudeFastWrapperPath,
    mcpServerCommand: [opencodeBinary, resolve(pluginDirectory, HONK_OPENCODE_MCP_SERVER_FILE)],
  });
...
const spawnOpenCode = (
  ...
  spawn(binary, ["serve", "--hostname", LOOPBACK_HOST, "--port", String(port)], {
    ...
    env: {
      ...
      // The V2 plane never reads OPENCODE_CONFIG; it loads the global config
      // DIRECTORY instead. Pointing it at the Honk-owned dir makes V2 load the
      // same overlay (V1-shaped configs are migrated) and Honk's plugin. The
      // V1 plane still reads the user's real config dir alongside this one.
      OPENCODE_CONFIG_DIR: dirname(configPath),
      OPENCODE_SERVER_PASSWORD: password,
      OPENCODE_CLIENT: "honk-cli",
      OPENCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS: "true",
      // Without this flag the registry omits the question tool, and the claude-code
      // plugin's version gate then silently drops its question proxy.
      OPENCODE_ENABLE_QUESTION_TOOL: "true",
      OPENCODE_EXPERIMENTAL_WEBSOCKETS: process.env.OPENCODE_EXPERIMENTAL_WEBSOCKETS ?? "true",
...
const requestAdmin = async (
  ...
  headers.set("authorization", `Bearer ${state.adminSecret}`);
  if (init?.body !== undefined && !headers.has("content-type")) {
    headers.set("content-type", "application/json");
  }
  return fetch(new URL(path, `${state.origin}/`), {
    ...init,
    headers,
...
const decodePairing = async (response: Response): Promise<PairingLink> => {
  ...
  // A plaintext LAN host omits the browser link: pairing needs a secure context.
  return { id, url: typeof url === "string" ? url : null, mobileUrl, expiresAt };
};
...
const printPairing = (pairing: PairingLink, publicUrl: string, includeQr: boolean): void => {
  process.stdout.write(
    [
      "Honk host is ready.",
      `Web: ${publicUrl}`,
      ...(pairing.url === null ? [] : [`Web attach: ${pairing.url}`]),
...
const start = async (options: CliOptions): Promise<void> => {
  ...
  if (existing !== null) {
    ...
    if (options.openBrowser && pairing.url !== null) await open(pairing.url);
    return;
  }
  ...
  const opencodeConfigPath = await writeHonkOpencodeConfig(statePath, binary);
  ...
  try {
    host = await startHonkHost({
      ...
      serverId: previous?.serverId ?? freshServerId(),
