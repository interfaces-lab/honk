...
- Put _why_ in fields (`reason`, `cause`, `signal`) or in the attached Cause, not in a subordinate clause in `message`.
...
- Add fields that explain _what_ failed, not only that something failed: `{ turnId }`, `{ sessionId }`, `{ path }`.
