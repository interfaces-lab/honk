    ...
    return this.keybindingsWriteLock.run(async () => {
      if (!(await pathExists(this.keybindingsConfigPath))) {
        await writeFileAtomically(
          this.keybindingsConfigPath,
          encodeKeybindingsConfig(DEFAULT_KEYBINDINGS),
        );
      ...
      const nextConfig = [
        ...customConfig,
        ...missingDefaults.filter((rule) => !duplicateDefaults.includes(rule)),
      ];
      const cappedConfig =
        nextConfig.length > MAX_KEYBINDINGS_COUNT
          ? nextConfig.slice(-MAX_KEYBINDINGS_COUNT)
