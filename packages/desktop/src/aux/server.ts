...
import type { ServerConfig, ServerConfigStreamEvent } from "@honk/shared/server-config";
import type { ClientOrchestrationCommand } from "@honk/shared/orchestration";
import { EDITORS, type EditorId } from "@honk/shared/editor";
import { EnvironmentId } from "@honk/shared/environment";
import { ProjectId as ProjectIdSchema, ThreadId } from "@honk/shared/base-schemas";
import {
  GitCheckoutInput,
...
import { createDesktopAuxSettingsService, type DesktopAuxSettingsService } from "./settings";
import { createDesktopAuxProjectsService, type DesktopAuxProjectsService } from "./projects";
import {
  createDesktopAuxTerminalService,
  type DesktopAuxTerminalRunInput,
  type DesktopAuxTerminalService,
  type DesktopAuxTerminalSessionInput,
} from "./terminal";
...
export interface DesktopAuxServerOptions {
  ...
  /** Main-process PTY seam for agent-started background jobs. */
  readonly terminal: DesktopAuxTerminalOptions;
}

export interface DesktopAuxTerminalOptions {
...
function requireGitCreateWorktreeInput(body: unknown): GitCreateWorktreeInput {
  ...
  return {
    ...
    ...(optionalString(record, "newBranch")
      ? { newBranch: optionalString(record, "newBranch") }
      : {}),
...
function requireGitRemoveWorktreeInput(body: unknown): GitRemoveWorktreeInput {
  ...
  return {
    cwd: requireString(record, "cwd"),
    path: requireString(record, "path"),
    ...(optionalBoolean(record, "force") !== undefined
...
function requireGitRunStackedActionInput(body: unknown): GitRunStackedActionInput {
  ...
  return {
    ...
    ...(optionalBoolean(record, "featureBranch") !== undefined
      ...
      : {}),
    ...(optionalStringArray(record, "filePaths")
      ? { filePaths: optionalStringArray(record, "filePaths") }
      : {}),
  };
}

function requireTerminalRunInput(body: unknown): DesktopAuxTerminalRunInput {
  const record = requireBodyRecord(body, "terminal run");
  return {
...
function requireTerminalSessionInput(body: unknown): DesktopAuxTerminalSessionInput {
  const record = requireBodyRecord(body, "terminal session");
  return {
    threadId: requireString(record, "threadId"),
...
export class DesktopAuxServer {
  ...
  private readonly terminal: DesktopAuxTerminalService;
  ...
  constructor(options: DesktopAuxServerOptions) {
    ...
    this.terminal = createDesktopAuxTerminalService({
      pty: options.terminal.pty,
      notifyOpen: options.terminal.notifyOpen,
  ...
  private jsonRoutes(): Map<string, JsonHandler> {
    return new Map<string, JsonHandler>([
      ...
      [
        "/projects/create",
        (body) => this.projects.createProject(requireProjectCreateCommand(body)),
      ],
      [
      ...
      [
        "/projects/delete",
        (body) => this.projects.deleteProject(requireProjectDeleteCommand(body)),
      ],
      [
        "/keybindings/upsert",
        async (body) => ({
      ...
      ["/git/discard-paths", (body) => this.git.discardPaths(requireGitDiscardPathsInput(body))],
      ["/git/file-patch", (body) => this.git.getFilePatch(requireGitFilePatchInput(body))],
      ["/git/file-image", (body) => this.git.getFileImage(requireGitFileImageInput(body))],
      ["/git/branches", (body) => this.git.listBranches(requireGitListBranchesInput(body))],
      [
      ...
      ["/git/branches/create", (body) => this.git.createBranch(requireGitCreateBranchInput(body))],
      ...
      // Enveloped: `handleJsonPost` turns a bare null into `{ ok: true }`, which would
      // read as "running" to the caller.
      [
        "/terminal/run",
        async (body) => ({ snapshot: await this.terminal.run(requireTerminalRunInput(body)) }),
      ],
      [
        "/terminal/status",
        async (body) => ({
          snapshot: await this.terminal.status(requireTerminalSessionInput(body)),
        }),
      ],
      [
        "/terminal/stop",
        async (body) => ({ snapshot: await this.terminal.stop(requireTerminalSessionInput(body)) }),
      ],
    ]);
  }
  ...
  private async handleGitStatusStream(
    ...
    sendSse(response);
    const unsubscribe = await this.git.subscribeStatus(
      { cwd, ...(scope ? { scope } : {}) },
      (event) => {
        writeSse(response, "message", event);
      },
