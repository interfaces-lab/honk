...
function remoteRefreshFailureDelay(consecutiveFailures: number): number {
  ...
  return Math.max(
    ...
    Math.min(backoffMs, GIT_STATUS_REFRESH_FAILURE_MAX_DELAY_MS),
  );
}
...
function sanitizeProgressText(value: string): string | null {
  ...
  return trimmed.length <= MAX_PROGRESS_TEXT_LENGTH
    ? trimmed
    : trimmed.slice(0, MAX_PROGRESS_TEXT_LENGTH).trimEnd();
}
...
const decodeGitHubJsonRecord = Schema.decodeUnknownOption(
...
async function statFile(
  filePath: string,
): Promise<{ readonly isFile: boolean; readonly size: number } | null> {
  try {
...
export class DesktopAuxGitService {
    ...
    try {
      ...
      if (beforeFetch?.worktreePath && beforeFetchPath !== rootWorktreePath) {
        ...
        return {
          ...
          worktreePath: beforeFetch.worktreePath,
        };
      }
      ...
      await this.materializePullRequestHeadBranch(
        input.cwd,
        pullRequestWithRemoteInfo,
      ...
      if (afterFetch?.worktreePath && afterFetchPath !== rootWorktreePath) {
        ...
        return {
          pullRequest,
          branch: localPullRequestBranch,
          worktreePath: afterFetch.worktreePath,
        };
      }
  ...
  private async executeGit(
    ...
    throw gitCommandError(
      ...
      stderr ||
        ...
        `${gitCommandLabel(args)} failed: code=${result.code}`,
    );
  }
  ...
  private async listRemoteNames(cwd: string): Promise<ReadonlyArray<string>> {
    return parseRemoteNamesInGitOrder(
      await this.gitStdout("GitCore.listRemoteNames", cwd, ["remote"]),
    );
  }
  ...
  private async refreshStatusUpstreamIfStale(cwd: string): Promise<void> {
    ...
    try {
      ...
      this.upstreamRefreshCache.set(
        cacheKey,
        Date.now() + STATUS_UPSTREAM_REFRESH_FAILURE_COOLDOWN_MS,
      );
    }
  }
  ...
  private async resolveBaseBranchForNoUpstream(
    cwd: string,
    branch: string,
  ): Promise<string | null> {
    ...
    const defaultBranch =
      primaryRemoteName === null
        ? null
        : await this.resolveDefaultBranchName(cwd, primaryRemoteName);
    const candidates = [
  ...
  private async readLocalStatus(cwd: string): Promise<GitStatusLocalResult> {
    ...
    const hostingProvider = details.isRepo
      ? await this.resolveHostingProvider(cwd, details.branch)
      : null;
    return {
  ...
  private async readConfigValue(cwd: string, key: string): Promise<string | null> {
    const trimmed = (
      await this.gitStdout("GitCore.readConfigValue", cwd, ["config", "--get", key], true)
    ).trim();
    return trimmed.length > 0 ? trimmed : null;
  }
  ...
  private async resolveHostingProvider(
    cwd: string,
    branch: string | null,
  ): Promise<GitStatusLocalResult["hostingProvider"] | null> {
  ...
  private async readBranchRecency(cwd: string): Promise<Map<string, number>> {
    const branchRecency = await this.executeGit(
  ...
  private async commit(
    ...
    const commitSha = (
      await this.gitStdout("GitCore.commit.revParseHead", cwd, ["rev-parse", "HEAD"])
    ).trim();
    return { commitSha };
  }
  ...
  private async pushCurrentBranch(
    ...
    if (hasNoLocalDelta) {
      ...
      const comparableBaseBranch = await this.resolveBaseBranchForNoUpstream(cwd, branch).catch(
        () => null,
      );
      if (comparableBaseBranch) {
        ...
        const hasRemoteBranch = await this.remoteBranchExists(cwd, publishRemoteName, branch).catch(
          () => false,
        );
        if (hasRemoteBranch) {
          return { status: "skipped_up_to_date", branch };
  ...
  private async pullCurrentBranch(cwd: string): Promise<GitPullResult> {
    ...
    const beforeSha = (
      await this.gitStdout("GitCore.pullCurrentBranch.beforeSha", cwd, ["rev-parse", "HEAD"], true)
    ).trim();
    ...
    const afterSha = (
      await this.gitStdout("GitCore.pullCurrentBranch.afterSha", cwd, ["rev-parse", "HEAD"], true)
    ).trim();
  ...
  private async ensureRemote(input: {
    readonly cwd: string;
    ...
    await this.executeGit("GitCore.ensureRemote.add", input.cwd, [
      "remote",
      "add",
      remoteName,
      input.url,
  ...
  private async fetchPullRequestBranch(input: {
    readonly cwd: string;
  ...
  private async getRepositoryCloneUrls(
    cwd: string,
    repository: string,
  ): Promise<GitHubRepositoryCloneUrls> {
    const result = await this.runGh(cwd, [
      "repo",
      "view",
      repository,
  ...
  private async configurePullRequestHeadUpstream(
    ...
    try {
      ...
      const preferredRemoteName =
        ...
        "fork";
      const remoteName = await this.ensureRemote({
        cwd,
  ...
  private async resolveCommitAndBranchSuggestion(input: {
    ...
    const firstChangedPath =
      context.stagedSummary.split(/\r?\n/g)[0]?.split(/\s+/g).pop() ?? "project files";
  ...
  private async runCommitStep(
    ...
    if (!suggestion) {
      ...
      suggestion =
        (await this.resolveCommitAndBranchSuggestion({
          cwd,
          branch,
          ...(commitMessage ? { commitMessage } : {}),
          ...(filePaths ? { filePaths } : {}),
  ...
  private async runPrStep(
    ...
    const latestSubject = (
      await this.gitStdout(
        "GitCore.runPrStep.latestSubject",
        cwd,
        ["log", "-1", "--pretty=%s"],
        true,
      )
      ...
      fail(
        command === "git"
          ? gitCommandError(
              ...
              options.cwd,
              args,
              `${gitCommandLabel(args)} timed out.`,
            )
