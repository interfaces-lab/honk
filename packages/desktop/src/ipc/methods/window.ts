...
// Setup's "Use defaults" needs a real folder to write into app settings, and the
// renderer has no filesystem. Main owns the one path it can honestly resolve.
export const getHomeDirectory = makeIpcMethod({
  channel: IpcChannels.GET_HOME_DIRECTORY_CHANNEL,
  payload: Schema.Void,
  result: Schema.String,
  handler: Effect.fn("desktop.ipc.window.getHomeDirectory")(function* () {
    const environment = yield* DesktopEnvironment.DesktopEnvironment;
