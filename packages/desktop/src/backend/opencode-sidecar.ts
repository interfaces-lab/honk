...
function opencodeBinaryCandidates(
  ...
  if (environment.isPackaged) {
    candidates.push(path.join(environment.resourcesPath, "opencode", "bin", "opencode.exe"));
...
export const resolveOpencodeBinary = Effect.fn("desktop.opencodeSidecar.resolveBinary")(function* (
  environment: DesktopEnvironment.DesktopEnvironmentShape,
...
const makeOpencodeSidecar = Effect.fn("desktop.opencodeSidecar.make")(function* () {
  ...
  const resolveSession = Effect.fn("desktop.opencodeSidecar.resolveSession")(function* () {
    ...
    // Resolved first: the overlay's MCP block spawns Honk's stdio server with this binary.
    const binaryPath = yield* resolveOpencodeBinary(environment).pipe(
      Effect.provideService(FileSystem.FileSystem, fileSystem),
    );
    const location = yield* writeHonkOpencodeConfig({
    ...
    // Honk's own aux API. The host plugin uses it for tools main has to serve, such
    // as the background terminal.
    const aux = yield* auxEndpoint.get;
    ...
    const session: SidecarSession = {
      ...
      env: {
        ...
        OPENCODE_CONFIG_DIR: location.configDir,
        ...
        no_proxy: mergeLoopbackNoProxy(process.env.no_proxy),
        ...(aux === null ? {} : { HONK_AUX_URL: aux.baseUrl, HONK_AUX_TOKEN: aux.bearer }),
      },
