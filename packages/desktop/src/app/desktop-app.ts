...
const startup = Effect.gen(function* () {
  ...
  const auxServer = createDesktopAuxServer({
    ...
    terminal: {
      pty,
      // Fire and forget: a window that is not up yet restores the tab from its route.
      notifyOpen: (request) => {
        Effect.runFork(electronWindow.sendAll(IpcChannels.TERMINAL_OPEN_CHANNEL, request));
      },
    },
  });
