...
import { DesktopEnvironment, type DesktopEnvironmentShape } from "../app/desktop-environment";

export const DESKTOP_SCHEME = "multi";
...
export interface ElectronProtocolShape {
  readonly registerSchemePrivileges: Effect.Effect<void>;
...
export class ElectronProtocol extends Context.Service<ElectronProtocol, ElectronProtocolShape>()(
  "multi/desktop/electron/Protocol",
) {}
...
const make = Effect.gen(function* () {
  ...
  const registerDesktopFileProtocol = Effect.gen(function* () {
    ...
    if (Option.isNone(staticRoot)) {
      yield* Effect.fail(new ElectronProtocolStaticBundleMissingError());
      return;
    }
  ...
  return ElectronProtocol.of({
    registerSchemePrivileges: registerDesktopSchemePrivileges,
