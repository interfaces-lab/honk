...
// Interactive tab clicks land on the thread's most complete scripted moment.
export function settledThreadScene(threadId: MarketingDemoThreadId): MarketingDemoScene {
  for (let index = marketingDemoSteps.length - 1; index >= 0; index -= 1) {
    const step = marketingDemoSteps[index];
    if (step && step.scene.activeThreadId === threadId) return step.scene;
  }
  return marketingDemoFinalScene;
}
